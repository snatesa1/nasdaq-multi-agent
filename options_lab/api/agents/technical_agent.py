...
Uses yfinance directly for OHLCV price data.
"""
...
class TechnicalAgent(BaseAgent):
    ...
    async def analyze(self, symbol: str, ohlcv_df: Optional[pd.DataFrame] = None, **kwargs) -> AgentResult:
        ...
        if ohlcv_df is None or ohlcv_df.empty:
            try:
                # yfinance download is synchronous, we run it in a thread or direct since it's quick
                ticker_data = yf.download(symbol, period="1y", progress=False)
                ohlcv_df = ticker_data
            except Exception as e:
                logger.error(f"Failed to fetch OHLCV data from yfinance for {symbol}: {e}")
        ...
        # Normalize column names to lowercase (supporting both multi-index and flat columns)
        if isinstance(df.columns, pd.MultiIndex):
            # Flatten columns if MultiIndex (sometimes yfinance returns multi-index if symbol is passed)
        ...
        # Ensure close is a Series/1D array
        ...
        indicators.update(self._calculate_hurst(df))
        momentum_regime = self._classify_momentum_regime(df, indicators)
